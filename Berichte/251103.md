...
| -------------- | -------------------------------------------------------- |
| `nc`           | Netcat – vielseitiges Netzwerk-Tool ("Swiss Army Knife") |
| `-v`           | Verbose – zeigt an, welche Ports offen sind              |
| `-z`           | Zero-I/O Mode – nur prüfen, keine Daten senden           |
| `192.168.1.52` | IP-Adresse der Metasploitable VM                         |
| `1-30`         | Portbereich, der gescannt wird (von Port 1 bis 30)       |
...
#### Nmap auf Metasploitable

Mit dem Befehl:
```
nmap 192.168.1.53
```
wird der Portscan auf Metasploitable durchgeführt.

![NMAP Portscan](../Bilder/NMAP%20Scan.png)



#### Nmap auf Kali Linux Lukas

Mit dem Befehl:
```
nmap 192.168.2.52
```
wird der Portscan auf das Kali von Lukas durchgeführt.

![NMAP Portscan Lukas](../Bilder/Kali%20Scan%20Luki.png)

#### Nmap auf Windows Rechner

Mit dem Befehl:
```
nmap 192.168.2.52
```
wird der Portscan auf den Windows Rechner durchgeführt.

![NMAP Portscan Windows](../Bilder/Nmap%20Portscan%20Windows.png)

#### Optionen siehe Cheatsheet

#### Nmap auf Default Gateway
...
![NMAP Portscan Default Gateway](../Bilder/Nmap%20Portscan%20Gateway.png)
...
Befehl:
```
sudo nmap -p- 192.168.1.x
```

Was es extra macht:
-p- scannt alle 65.535 Ports statt nur die 1.000 Standardports.

#### Ping-Scan/Host-Discovery überspringen

Befehl:
```
sudo nmap -Pn 192.168.1.x
```

Was es extra macht:
-Pn sagt Nmap: „Geh davon aus, dass der Host aktiv ist.“
→ Keine ICMP/Ping-Abfrage, direkt Portscan.
...
Was es extra macht:
Ermittelt Versionen von Diensten, z. B.:
...
Was es extra macht:
Versucht anhand von Netzwerkmerkmalen zu erkennen:

- OS-Typ
- OS-Version
- Netzwerkstack-Verhalten
...
Beispiel (schnell):
```
sudo nmap -T4 192.168.1.x
```

Beispiel (extrem schnell, evtl. ungenau):
```
sudo nmap -T5 192.168.1.x
```

Was es extra macht:

- T4 beschleunigt das Scannen deutlich bei stabilen Netzwerken.
- T5 ist aggressiv, kann aber Pakete verlieren.

#### Ausgabe in Dateien

Befehl:
```
sudo nmap -oN scan.txt -oX scan.xml 192.168.1.x
```

Was es extra macht:

- oN = normale Textausgabe
- oX = XML-Ausgabe (zum Weiterverarbeiten)

#### Die Option --reason

Befehl:
```
sudo nmap --reason 192.168.1.x
```

Was es extra macht:
Zeigt warum ein Port als

- open
- closed
- filtered
- klassifiziert wurde (z. B. SYN/ACK empfangen).

#### Nur die N häufigsten Ports scannen (Geschwindigkeitsgewinn!)

Befehl:
```
sudo nmap --top-ports 100 192.168.1.x
```

Was es extra macht:
Scant nur die 100 am häufigsten verwendeten Ports statt 1.000 oder 65.535.
